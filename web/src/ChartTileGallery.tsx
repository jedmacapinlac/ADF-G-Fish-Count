...
/** A horizontally scrolling row of big square chart tiles — shared by every
 *  tab that wants one (Run Overview, Daily Counts, ...).
 *
 *  A tile is clickable: it mounts a second, larger copy of the same chart in
 *  a modal (its own fetch, its own hover/selector state) rather than
 *  resizing the small one in place, so the small tile keeps rendering
 *  underneath while the modal is open. */
export default function ChartTileGallery({ charts }: Props) {
